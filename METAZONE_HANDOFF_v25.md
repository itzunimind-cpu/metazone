...
| editor.html | v7.2 | Session 41 | Pan/path-finish input overhaul — middle-mouse-drag pans in both modes, preview-mode left-click/touch pans directly (space+left-click removed), right-click now finishes a path (dblclick kept as fallback). Plus a later fix: the shared delete-warning confirm modal (`_dwmConfirm()` was nulling its own callback before invoking it — silently broke CLEAR MAP, clear match map, and every delete-tournament/day/match/team confirm); `clearMap()` also now purges `self_elims` and `annotations` (elim log + notes), previously untouched. See Section 8 changelog. |
...
| `clearMap()` | Confirm-gated via `showDeleteWarning`. Clears shapes/zones, team wipe fields, match_player loss/death fields, `matches.duration_seconds`. **Session 41 fix:** also now clears `self_elims` (Self Elim log entries) and `annotations` (saved notes) — both DB tables and their local arrays — previously left untouched so those events would reappear after a "clear". |
