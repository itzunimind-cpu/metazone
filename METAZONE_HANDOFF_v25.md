...
- `#squad-panel` — same id as before v3.0, restyled into a floating top-right panel instead of a docked sidebar. Map Opacity moved to the top; Enemy Teams is now collapsible (`toggleEnemyTeams()`, collapsed by default); Event Log card is now just the label + `#vod-event-log` list — no controls (the 4 event buttons moved to `#vod-left-tools`, see below).
...
- `#vod-context-chip` — new, replaces the old `#match-context-bar` breadcrumb. Read-only canvas overlay, top-left of `#vod-stage`: **Tournament › Day › Match › Map only.** First held the data-status chip + video-sync indicator + match timer too, then per a decluttering follow-up those were pulled back out (see below) — the chip is now intentionally minimal.
...
- `#tb-match-time` (match elapsed timer) — relocated again, into a new `#sp-match-data` strip at the very top of `#squad-panel` (above Map Opacity). Same id, same `updateMatchTimeDisplay()`/`_setSyncLost()` wiring.
