...
**Version:** v40 — Session 40. vod.html: removed in-page tournament/day/match switching UI (the three toolbar dropdowns) — match navigation now lives only on the dashboard, deep-linking via URL params. editor.html: fixed a cross-page bug where openInVOD() sent the wrong URL param names. index.html: added a delete-match action to the dashboard's match card, reusing editor.html's cascade-delete pattern.
...
|------|---------|--------------|-------|
| index.html | v9.6 | Session 40 | Added delete-match action to dashboard match card (cascade-delete, reuses existing delete-warning modal pattern). See Section 8 changelog. |
...
| vod.html | v5.0 | Session 40 | Removed in-page tournament/day/match switching dropdowns — navigation is dashboard-only now, deep-linking via URL params unchanged. See Section 8 changelog. |
